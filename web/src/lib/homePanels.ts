 ...
 *   this week   which pages it actually learned      (graph nodes, by mtime)
 *   throughput  how much came in, and through what   (jobs)
...
export interface ThroughputDay {
  /** Days back from today; the array runs oldest to newest so it plots left to right. */
  readonly ago: number
...
/**
 * Jobs that finished per day, split by what put them there. Every day in the window gets a
 * bar, including the empty ones - a chart that skips its quiet days reports a busier
 * pipeline than the one that ran.
 */
export function throughput(jobs: readonly Job[], now: number, days = THROUGHPUT_DAYS): ThroughputDay[] {
  const midnight = new Date(now).setHours(0, 0, 0, 0)
  const out: ThroughputDay[] = []
  const counts = new Map<number, { drop: number; watcher: number; bot: number }>()
  for (const j of jobs) {
    const when = j.finished_at ?? j.created_at
    const ago = Math.floor((midnight - new Date(when).setHours(0, 0, 0, 0)) / DAY_MS)
    if (ago < 0 || ago >= days) continue
    const bucket = counts.get(ago) ?? { drop: 0, watcher: 0, bot: 0 }
    // `drop` and `url` are both a person adding something by hand - a file or a pasted
    // link - so they count as one channel; the two automatic ones each keep their own.
    if (j.source === 'watch') bucket.watcher++
    else if (j.source === 'telegram') bucket.bot++
    else bucket.drop++
    counts.set(ago, bucket)
  }
  for (let ago = days - 1; ago >= 0; ago--) {
    const c = counts.get(ago) ?? { drop: 0, watcher: 0, bot: 0 }
    out.push({ ago, ...c })
  }
  return out
}
