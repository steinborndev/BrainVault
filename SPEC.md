...
2. **Rerank (lokal):** Kosinus-Rerank über ollama (`nomic-embed-text`, `127.0.0.1:11434`). Voraussetzungen: Der Embed-Cache wird beim Index-Bau vorgewärmt (die Query-Zeit bleibt lesend), und die Sandbox des Query-Profils erhält eine eng gefasste localhost-Ausnahme für den ollama-Port; nach jeder Änderung an der Permission-Verdrahtung gilt Hard Rule 4 (permprobe). Fällt ollama aus, degradiert das Retrieval auf BM25-Reihenfolge statt zu scheitern.
3. **Echte Kontext-Prefixes (Egress, opt-in):** Prefix-Generierung über die Service-Credential. Nur hinter einem expliziten Setting (Default aus), denn dabei verlassen Wiki-Seiteninhalte die Maschine — der Service übernimmt die Consent-Schranke, die der Vault selbst mit `--allow-egress` zieht, statt sie stillschweigend zu umgehen.
