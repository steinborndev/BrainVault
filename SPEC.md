...
**Persistenz (Stufe b):** Eine SQLite-Tabelle `maintenance_state` (letzter Durchlauf/Check je Bereich, letzte Zusammenfassung; rein operational, Hard Rule 1 unberührt) macht "zuletzt erledigt" und das "N Punkte fällig"-Badge im Übersichts-Tab Restart-fest. Der optionale Wochen-Lint aus 6.4 geht hier auf: ein Scheduler führt nur den Check (und optional den Lint) aus und setzt Punkte auf die Liste - nie ungefragt Entscheidungsschritte.
...
- **Stufe (b):** Statusmodell + "Was steht an"-Kopf über den Karten, `maintenance_state`, Badge im Übersichts-Tab.
- **Stufe (c):** Geführter Durchlauf (Queue über dem `MaintenanceRunner`, Wizard-UI mit Automatik-/Entscheidungsschritten, Folge-Backfill automatisch), optionaler Check-Turnus. `contextual-prefix.py --all` arbeitet inkrementell (nur geänderte Seiten), der BM25-Rebuild ist reines Python und billig.
